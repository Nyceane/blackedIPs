...
module.exports = class SpaceAndTimeSDK {
    constructor() {
        this.baseUrl = config.space_and_time.url;
    } 
    ...
    async AuthenticateUser(privateKeyArg = "", publicKeyArg = "") {
        ...
        let publicKey = (config.space_and_time.public_key === undefined) ? mainPublicKey : config.space_and_time.public_key;
        let privateKey = (config.space_and_time.private_key === undefined) ? mainPrivateKey : config.space_and_time.private_key;
    ...
    async Authenticate(privateKey, publicKey, prefix = "") {
        ...
        let [ authCodeResponse, authCodeError ] = await this.#generateAuthCode(userId);
        ...
        let [ tokenResponse, tokenError ] = await this.#generateToken(userId, authCodeResponse, signature, publicKey);
